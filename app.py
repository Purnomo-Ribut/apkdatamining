...
app = Flask(__name__)
app.secret_key = 'super secret' #session dalam flask
...
def index():
	...
	if request.method == 'POST':
		...
		items = ReadData('dataset.txt') # memasukkan dataset ke dalam items

		#menjadikan inputan user type intejer
		...
		accuracy2, maxi, count, neighbors = Classify(newItem, k, items) #k = fitur keputusannya, item berisi semua fitur, new item = value dari item

		skor = pd.DataFrame(neighbors, columns = ['Skor' , 'Hasil Keputusan']) #pandas.
